...
def launch_training_task(
    ...
    val_dataloader = None
    tracked_val_samples = None  # list of dicts (raw samples)
    tracked_ids = set()
    ...
    if val_dataset is not None:
        ...
        if args is not None and getattr(args, "val_dataset_metadata_path", None):
            ...
            tracked_ids = {s.get("sample_id", f"idx_{i}") for i, s in enumerate(tracked_val_samples)}
    ...
    global_step = 0
    t0 = time.time()
    eval_every = int(getattr(args, "eval_every_steps", 0) or 0)
    ...
    for epoch_id in range(num_epochs):
        for data in tqdm(dataloader):
            ...
            if eval_every and (global_step % eval_every == 0) and (global_step > 0):
                ...
                if (vloss is not None) and accelerator.is_main_process:
